...
class MLPmodel(nn.Module):
    def __init__(
        ...
        self._output_mlp = MLP(
            in_size = self.total_candidate_dim,
    ...
    def forward(
        ...
        # embeddings_awaitable = self._embedding_module(kjt)
        # embeddings = embeddings_awaitable.wait()
        # print("[debugging] without wait will cause error?")
        embeddings = self._embedding_module(kjt)
        ...
        logits = self._output_mlp(concatenated_candidate_features)       # [batch_size, mlp_out_dim]
